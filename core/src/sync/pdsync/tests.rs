...
use crate::sync::personal::{apply_personal_remote, delete_personal, is_tombstone, put_personal};
...
fn category_friend() -> &'static Category {
    CATEGORIES.iter().find(|c| c.name == "ct:friend").unwrap()
}
...
fn collect_incremental_filters_by_known_vv() {
    ...
    let known_full: VersionVector = [(NODE_A.to_string(), 2), (NODE_B.to_string(), 1)]
        .into_iter()
        .collect();
...
fn apply_data_is_idempotent() {
    ...
    let meta = get_personal_meta(&a, &format!("{FRIEND_PREFIX}a"))
        .unwrap()
        .unwrap();
...
fn build_parse_roundtrip_need_and_data() {
    let mut s = MemoryStorage::new();
    put_personal(&mut s, NODE_A, &format!("{FRIEND_PREFIX}a"), r#""v""#, 1000).unwrap();
...
fn exchange(
    ...
    for category in CATEGORIES {
        ...
        match diff_category(&local_vv, &remote_vv) {
            ...
            DiffOutcome::Equal => {
                ...
                if let Ok(tombs) = collect_tombstones_after(receiver, category, exclude, hello_ack)
                {
                    *sender_seen = tombs
                        .iter()
                        .filter_map(|r| r.dseq)
    ...
    for body in out {
        if let Some((cat_name, known_vv, need_ack)) = parse_need(&body) {
            ...
            let records = collect_incremental(sender, cat, &known_vv, exclude, need_ack).unwrap();
...
fn two_device_exchange_converges() {
    ...
    assert_eq!(
        collect_category_vv(&b, category_friend(), None)
            .unwrap()
            .get(NODE_A),
        Some(&3)
    );
    ...
    // A 也有 b0 了
    assert_eq!(
        collect_category_vv(&a, category_friend(), None)
            .unwrap()
            .get(NODE_B),
        Some(&1)
    );
...
fn two_device_exchange_converges_profile_and_conv() {
    ...
    let hello_a2 = build_hello(&a, 2_592_000_000, 500, "eager", None, None).unwrap();
    assert!(
        exchange_simple(&a, &mut b, &hello_a2, None).is_empty(),
        "profile/conv 收敛后无增量"
    );
}
...
fn collect_message_window_age_cutoff_keeps_in_window_messages() {
    ...
    for (i, off) in offsets.iter().enumerate() {
        let key = crate::message::types::message_key("personal", "c1", now + off, &format!("m{i}"));
...
fn two_device_message_window_converges() {
    ...
    for i in 0..2 {
        let key =
            crate::message::types::message_key("personal", "c1", now - 1000, &format!("a{i}"));
    ...
    for (s, name) in [(&a, "a"), (&b, "b")] {
        for id in ["a0", "a1", "b0"] {
            ...
            assert!(s.get(&key).unwrap().is_some(), "{name} 缺少消息 {id}");
        }
    }
}
...
fn self_friend_record_excluded_from_sync() {
    ...
    assert!(
        folded_b.get(NODE_B).is_none(),
        "B 排除自记录后 ct:friend 为空"
    );
    ...
    assert!(
        a_self.contains("peer-a"),
        "A 自记录 peer 保持指向 A 设备: {a_self}"
    );
    let b_self = b.get(&self_key).unwrap().expect("B 自记录仍在");
    assert!(
        b_self.contains("peer-b"),
        "B 自记录 peer 保持指向 B 设备: {b_self}"
    );
    // 普通朋友记录照常同步到 B
    ...
    assert!(
        exchange_simple(&a, &mut b, &hello_a2, Some(&self_key)).is_empty(),
        "A→B 收敛无增量"
    );
    let hello_b2 = build_hello(&b, 2_592_000_000, 500, "eager", Some(&self_key), None).unwrap();
    assert!(
        exchange_simple(&b, &mut a, &hello_b2, Some(&self_key)).is_empty(),
        "B→A 收敛无增量"
    );
}
...
fn p6_declared_collection_syncs_end_to_end() {
    ...
    assert!(
        cats.iter().any(|(name, _)| name == "pdecl"),
        "hello 含 pdecl"
    );
    ...
    // B 获得声明与数据
    let decl_b =
        crate::plugindata::resolve(&b, "ai-chat:conversations", None).expect("B 端声明已同步");
