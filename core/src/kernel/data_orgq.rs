...
impl Kernel {
    ...
    fn orgq_target_peer<S: StorageBackend>(
        &self,
        org_id: &str,
        target_root_id: &str,
        storage: &S,
    ...
    /// 超时/投递失败/p2p 未启动（调用方回退缓存语义）。
    fn orgq_deliver_and_wait(
        &self,
        org_id: &str,
        target_root_id: &str,
        request_id: &str,
        ...
        let Some(peer) = self.orgq_target_peer(org_id, target_root_id, self.require_storage()?)
        else {
            return Ok(false);
        };
        let envelope = match self.build_dm_envelope(KIND_ORGQ_REQ, target_root_id, body) {
            Ok(e) => e,
        ...
        let delivered = self
            .runtime
            .handle()
            .block_on(node.dm_direct(&peer, envelope))
            .ok()
            .flatten()
    ...
    pub(crate) fn data_orgq_query(
        ...
        let arrived = self.orgq_deliver_and_wait(org_id, target_root_id, &request_id, body)?;
        if !arrived {
