...
fn lenient_sync_strategy<'de, D>(
    deserializer: D,
) -> std::result::Result<Option<SyncStrategy>, D::Error>
where
    D: Deserializer<'de>,
{
...
impl CollectionSchemaRecord {
    ...
    fn policy(&self) -> Option<ResolvedCollectionPolicy> {
        self.sync_strategy
            .map(|sync_strategy| ResolvedCollectionPolicy {
                sync_strategy,
...
pub enum SchemaError {
    /// syncStrategy 缺失或非法。
    #[error(
        "Invalid collection schema: syncStrategy must be declared as \"append-only\" or \"lww\""
    )]
    InvalidSyncStrategy,
...
pub fn resolve_schema_declaration(
    ...
    let sync_strategy = declaration
        .and_then(|d| d.sync_strategy)
        .ok_or(SchemaError::InvalidSyncStrategy)?;
...
pub fn declare_collection_schema<S: StorageBackend>(
    ...
    storage.put(
        &collection_schema_key(domain, collection),
