...
fn declare_sync<S: StorageBackend>(s: &mut S, name: &str) -> CollectionDeclaration {
    declare(s, "ai-chat", input(name), 1000).unwrap()
}

#[test]
fn declare_persists_and_is_idempotent() {
    let mut s = MemoryStorage::new();
    let d1 = declare(&mut s, "ai-chat", input("ai-chat:conversations"), 1000).unwrap();
    ...
    let d2 = declare(&mut s, "ai-chat", input("ai-chat:conversations"), 2000).unwrap();
...
fn declare_conflicting_strategy_rejected() {
    ...
    let err = declare(&mut s, "ai-chat", conflict, 2000).unwrap_err();
...
fn declare_new_version_creates_independent_namespace() {
    ...
    let d2 = declare(&mut s, "ai-chat", v2, 2000).unwrap();
...
fn declare_validation() {
    ...
    // 缺 collection 段
    assert!(matches!(
        declare(&mut s, "ai-chat", input("ai-chat"), 1).unwrap_err(),
        PlugindataError::InvalidName(_)
    ));
    // collection 段字符集
    assert!(matches!(
        declare(&mut s, "ai-chat", input("ai-chat:has space"), 1).unwrap_err(),
        PlugindataError::InvalidName(_)
    ));
    // @ 保留
    assert!(matches!(
        declare(&mut s, "ai-chat", input("ai-chat:a@b"), 1).unwrap_err(),
        PlugindataError::InvalidName(_)
    ));
    // 插件前缀归属
    assert!(matches!(
        declare(&mut s, "ai-chat", input("other-plugin:x"), 1).unwrap_err(),
    ...
    assert!(matches!(
        declare(&mut s, "ai-chat", bad_version, 1).unwrap_err(),
...
fn resolve_defaults_to_latest_generation() {
    ...
    declare(&mut s, "ai-chat", v2, 2000).unwrap();
...
fn devices_residency_matrix() {
    ...
    let all = declare(&mut s, "ai-chat", base(Devices::All), 1).unwrap();
    let backup = declare(
        &mut s,
        "ai-chat",
