...
fn org_group_move_cross_level() {
    ...
    ContactService::move_org_group(&mut s, ORG, &branch.id, &hq.id, 0).unwrap();
    let view = ContactService::overview(&s, ORG).unwrap();
    ...
    ContactService::move_org_group(&mut s, ORG, &tech.id, &branch.id, 0).unwrap();
    let view = ContactService::overview(&s, ORG).unwrap();
    assert_eq!(
        tree_ids(&view.group_tree[0].children),
    ...
    ContactService::move_org_group(&mut s, ORG, &tech.id, "", 99).unwrap();
    let view = ContactService::overview(&s, ORG).unwrap();
    assert_eq!(tree_ids(&view.group_tree), vec![hq.id.as_str(), tech.id.as_str()]);
    assert!(view.group_tree[0].children[0].children.is_empty());

    // 目标父不存在 → 静默忽略（树不变）
    ContactService::move_org_group(&mut s, ORG, &tech.id, "og-x", 0).unwrap();
    let view = ContactService::overview(&s, ORG).unwrap();
    assert_eq!(tree_ids(&view.group_tree), vec![hq.id.as_str(), tech.id.as_str()]);

    // 防环：总部移入自己的子树（市场部）或自身 → 静默忽略（树不变）
    ContactService::move_org_group(&mut s, ORG, &hq.id, &market.id, 0).unwrap();
    ContactService::move_org_group(&mut s, ORG, &hq.id, &hq.id, 0).unwrap();
    let view = ContactService::overview(&s, ORG).unwrap();
    assert_eq!(tree_ids(&view.group_tree), vec![hq.id.as_str(), tech.id.as_str()]);
    assert_eq!(
        tree_ids(&view.group_tree[0].children),
    ...
    // 源节点不存在 → 静默忽略
    ContactService::move_org_group(&mut s, ORG, "og-x", "", 0).unwrap();
}
...
fn org_group_move_cross_level_same_parent_matches_splice() {
    ...
    ContactService::move_org_group(&mut s, ORG, &tech.id, &hq.id, 2).unwrap();
    // 前移：财务部 → 下标 0 → [财务部, 市场部, 技术部]
    ContactService::move_org_group(&mut s, ORG, &fin.id, &hq.id, 0).unwrap();
    // toIndex == len 移到末尾：市场部 → 3 → [财务部, 技术部, 市场部]
    ContactService::move_org_group(&mut s, ORG, &market.id, &hq.id, 3).unwrap();
