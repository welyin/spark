...
fn dm_signing_payload_and_signature_match_vector() {
    for v in vectors() {
        let desc = v["description"].as_str().unwrap_or(" unnamed vector");
        ...
        assert_eq!(
            ...
            from,
            "{desc}: rootId = sha256hex(pubKey)"
        );

        // 签名载荷确定输出
        let payload = build_signing_payload(kind, from, to, ts, body);
        assert_eq!(payload, v["payload"].as_str().unwrap(), "{desc}: 签名载荷精确匹配");
        ...
        let verified = verify_envelope(&envelope, to, ts).expect("{desc}: vector 信封应校验通过");
        assert_eq!(verified.kind, kind, "{desc}: kind");
        assert_eq!(verified.from, from, "{desc}: from");
        assert_eq!(verified.ts, ts, "{desc}: ts");
        assert_eq!(verified.body, *body, "{desc}: body");
    }
}
