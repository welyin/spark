...
fn build_app_message_validation() {
    ...
    for bad in [
        ...
        assert!(matches!(
            AppMessageService::build_app_message("weibo-core", bad, None, "m1".into(), NOW),
    ...
    assert!(matches!(
        AppMessageService::build_app_message("weibo-core", payload(&long), None, "m1".into(), NOW),
        Err(MessageError::SummaryTooLong)
    ));
    // 恰好 200 字符放行
    let exact = "好".repeat(200);
    assert!(
        AppMessageService::build_app_message("weibo-core", payload(&exact), None, "m1".into(), NOW)
            .is_ok()
    );

    // pluginId 字符集：大写 / 含冒号 / 空串 / 超长 → InvalidPluginId
    for bad in ["Weibo", "plugin:weibo-core", "", "a".repeat(65).as_str()] {
        assert!(matches!(
...
fn service_append_list_mark_read_delete() {
    ...
    let conv = AppMessageService::ensure_app_conversation(&mut storage, PERSONAL, "weibo-core", NOW).unwrap();
    assert_eq!(conv.id, app_conversation_id("weibo-core"));
    assert_eq!(conv.id, "app:weibo-core");
    assert_eq!(conv.kind, ConversationKind::App);
    let again =
        AppMessageService::ensure_app_conversation(&mut storage, PERSONAL, "weibo-core", NOW + 1).unwrap();
    ...
    // 列表：时间升序
    let messages = AppMessageService::list_app_messages(&storage, PERSONAL, "weibo-core").unwrap();
    ...
    // 空间隔离：其他空间看不到
    assert!(
        AppMessageService::list_app_messages(&storage, "org:org_0123456789abcdef", "weibo-core")
            .unwrap()
            .is_empty()
    );

    // 标记已读：未读清零 + 消息 read 批量置位（幂等）
    AppMessageService::mark_app_read(&mut storage, PERSONAL, "weibo-core").unwrap();
    let conv = MessageService::get_conversation(&storage, PERSONAL, &conv.id)
        .unwrap()
        .unwrap();
    assert_eq!(conv.unread_count, 0);
    assert!(
        AppMessageService::list_app_messages(&storage, PERSONAL, "weibo-core")
            .unwrap()
            .iter()
            .all(|m| m.read)
    );
    ...
    // 删除会话：消息与会话一并删除
    AppMessageService::delete_app_conversation(&mut storage, PERSONAL, "weibo-core").unwrap();
    assert!(
        AppMessageService::list_app_messages(&storage, PERSONAL, "weibo-core")
            .unwrap()
            .is_empty()
...
fn rate_limiter_window_and_isolation() {
    ...
    for _ in 0..APP_MSG_RATE_LIMIT {
        assert!(limiter.check(PERSONAL, "weibo-core", NOW));
    }
    assert!(!limiter.check(PERSONAL, "weibo-core", NOW + 1));
    assert!(!limiter.check(PERSONAL, "weibo-core", NOW + 2));
    assert_eq!(limiter.rejected_count(PERSONAL, "weibo-core"), 2);

    // 其他插件 / 其他空间互不影响
    assert!(limiter.check(PERSONAL, "other-app", NOW + 3));
    assert!(limiter.check("org:org_0123456789abcdef", "weibo-core", NOW + 3));

    // 窗口过期重置（拒绝计数保留）
    assert!(limiter.check(PERSONAL, "weibo-core", NOW + 60_000));
    assert_eq!(limiter.rejected_count(PERSONAL, "weibo-core"), 2);
}
...
fn app_send_list_mark_read_delete_flow() {
    ...
    let card: spark_core::message::AppMessageCard =
        serde_json::from_value(json!({ "viewId": "notice-card", "data": { "level": "info" } }))
            .unwrap();
    let view = kernel
        .message_app_send(PERSONAL, "weibo-core", payload("欢迎"), Some(card.clone()))
        .unwrap();
    ...
    kernel
        .message_app_send(PERSONAL, "weibo-core", payload("第二条"), None)
        .unwrap();
    let messages = kernel.message_app_list(PERSONAL, "weibo-core").unwrap();
    assert_eq!(messages.len(), 2);
    assert!(messages.iter().all(|m| m.status == "local"));
    kernel.message_app_mark_read(PERSONAL, "weibo-core").unwrap();
    assert_eq!(
    ...
    assert!(
        kernel
            .message_app_list(PERSONAL, "weibo-core")
            .unwrap()
            .iter()
            .all(|m| m.read)
    );
    ...
    assert!(
        kernel
            .message_app_send(PERSONAL, "weibo-core", json!({ "kind": "notice" }), None)
            .is_err()
    );
    assert_eq!(kernel.message_app_list(PERSONAL, "weibo-core").unwrap().len(), 2);

    // 删除会话
    kernel
        .message_app_delete_conversation(PERSONAL, "weibo-core")
        .unwrap();
    assert!(kernel.message_app_list(PERSONAL, "weibo-core").unwrap().is_empty());
...
fn app_send_rate_limited_and_counted() {
    ...
    assert!(
        kernel
            .message_app_send(PERSONAL, "weibo-core", json!({}), None)
            .is_err()
    );
    for i in 0..APP_MSG_RATE_LIMIT {
        kernel
            .message_app_send(PERSONAL, "weibo-core", payload(&format!("第{i}条")), None)
            .unwrap();
    }
    // 第 11 条：rate-limited，不落库、未读不变、拒绝计数 +1
    let err = kernel
        .message_app_send(PERSONAL, "weibo-core", payload("超限"), None)
        .unwrap_err();
    assert!(
        err.to_string().contains("rate-limited"),
        "限流错误应含 reason：{err}"
    );
    assert_eq!(kernel.message_app_rate_rejected(PERSONAL, "weibo-core"), 1);
    assert_eq!(
        kernel.message_app_list(PERSONAL, "weibo-core").unwrap().len() as u32,
        APP_MSG_RATE_LIMIT
    );
    assert_eq!(
        kernel.message_list_conversations(PERSONAL).unwrap()[0].unread_count,
...
fn app_messages_hydrate_after_restart() {
    ...
    kernel.stop_p2p().unwrap();
    kernel
        .message_app_send(PERSONAL, "weibo-core", payload("重启前的消息"), None)
        .unwrap();
    kernel.shutdown().unwrap();
