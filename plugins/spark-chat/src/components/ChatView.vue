...
<template>
  <section v-if="conversation" class="chat-view">
    ...
    <div ref="scrollRef" class="msg-scroll">
      <!-- 应用会话（服务号模型 §20）：v1 由壳层应用会话挂载区呈现
           （communication §4.2 壳层保留面），插件内仅占位说明 -->
      <template v-if="isAppConversation">
        <el-empty :image-size="80" description="应用会话由壳层挂载区呈现" class="chat-empty" />
      </template>
    ...
    <!-- v1：联系人资料卡抽屉 / 组织邀请抽屉未迁移（壳层通讯录/组织面职责，
         迁移清单记录在案）；头像点击与邀请卡片点击暂为提示 -->
  </section>
</template>
  ...
  setup(props) {
    ...
    const renderItems = computed<RenderItem[]>(() => {
      ...
      for (const msg of messages.value) {
        // 个人过滤规则（communication §4.3）：渲染前过滤，纯本地视图行为
        if (isMessageFiltered(msg)) continue;
    ...
    // ---- v1 未迁移的壳层面（迁移清单记录在案）：资料卡抽屉/组织邀请抽屉 ----
    function openProfileCard(_rootId: string) {
      ElMessage.info('资料卡将在后续版本提供');
    }

    function openOrgInvite(_payload: { inviteId: string; orgId: string; title: string; description: string }) {
      ElMessage.info('组织邀请确认将在后续版本提供');
    }
