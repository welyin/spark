...
// 内核通讯录/组织接口由 sdk-host 适配（SDK 未绑定时为 undefined，全调用点
// 守卫 + catch 静默）；re-export 保持 queries/requests/tags/groups 的 import 路径
export { contactsApi, organizationApi } from '../sdk-host';
...
/**
 * 演示数据开关：插件版恒 false（无壳层 mock 演示链路；非桥环境退化种子
 * 数据由 isTauri 守卫承担——SDK 未绑定即种子，与壳层「非 Tauri」同口径）。
 */
