// spark-contacts store 事件单测（功能对等迁移自壳层 mock/contacts/store.test）：
// FriendRequestReceived 按 id upsert、FriendRequestSent 投递终态回写 outbox、
// FriendProfileUpdated 资料同步、OrgInviteUpdated 组织邀请回执合入组织空间
// 「我发出的邀请」。数据面无 SDK 绑定时退化为本地种子数据（isTauri 守卫口径）。
import { describe, it, expect } from 'vitest';
import type { FriendRequestDto, OrgInviteRecordDto } from '../src/host-types';
import { handleContactsP2pEvent, contactsOf } from '../src/store/store';
import { emptyProfile } from '../src/store/types';
