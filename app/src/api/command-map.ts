...
export const COMMAND_MAP: Record<string, string> = {
  ...
  'root-backup-payload-qr': 'root_backup_payload_qr',
  'root-reveal-mnemonic': 'root_reveal_mnemonic',
  'root-change-password': 'root_change_password',
  ...
  'community-invite-create': 'community_create_invite',
  'community-invite-send': 'community_send_invite',
  'community-invite-accept': 'community_accept_invite',
  'community-list-members': 'community_list_members',
  // 通讯录
  ...
  // relay 状态页/托管向导（relay-implementation §3 U1/U2；只读）
  ...
  // 内容面 blob（public-topics §七 sdk.content 薄壳命令）
  'content-save-blob': 'content_save_blob',
  'content-read-blob': 'content_read_blob',
  'content-fetch-blob': 'content_fetch_blob',
  'content-list-blobs': 'content_list_blobs',
  'content-pin-root': 'content_pin_root',
  'content-unpin-root': 'content_unpin_root',
  'content-gc-sweep': 'content_gc_sweep',
  // community-affairs §7.2：sdk.affairs / sdk.credentials / sdk.policy 薄壳命令
  'affairs-follow': 'plugin_affairs_follow',
  'affairs-unfollow': 'plugin_affairs_unfollow',
  'affairs-list-followed': 'plugin_affairs_list_followed',
  'affairs-submit-op': 'plugin_affairs_submit_op',
  'affairs-read-log': 'plugin_affairs_read_log',
  'affairs-read-rules': 'plugin_affairs_read_rules',
  'affairs-read-resolution': 'plugin_affairs_read_resolution',
  'affairs-ladder-status': 'plugin_affairs_ladder_status',
  'affairs-public-profile': 'plugin_affairs_public_profile',
  'affairs-snapshot-payload': 'plugin_affairs_snapshot_payload',
  'affairs-read-exec': 'plugin_affairs_read_exec',
  'affairs-org-effects': 'plugin_affairs_org_effects',
  'affairs-apply-org-effects': 'plugin_affairs_apply_org_effects',
  'credentials-list-held': 'plugin_credentials_list_held',
  'credentials-present-holder-proof': 'plugin_credentials_present_holder_proof',
  'credentials-query-verifiers': 'plugin_credentials_query_verifiers',
  'credentials-verify': 'plugin_credentials_verify',
  'credentials-query-revocations': 'plugin_credentials_query_revocations',
...
export const ARG_NAMES: Record<string, string[]> = {
  ...
  'root-change-password': ['oldPassword', 'newPassword'],
  'root-backup-payload-qr': ['password'],
  'root-sign': ['payload'],
  ...
  // 共同体域（命令形参 camelCase；Tauri 自动映射到 snake_case）
  'community-invite-create': ['communityOrgId'],
  'community-invite-send': ['communityOrgId', 'code', 'toOrgAddress', 'recipientDomainId', 'gatewayPeerId', 'gatewayAddresses'],
  'community-invite-accept': ['joinerOrgId', 'code', 'publishBinding'],
  'community-list-members': ['communityOrgId'],
  ...
  // 内容面 blob（命令形参 camelCase；Tauri 自动映射到 snake_case）
  'content-save-blob': ['dataBase64'],
  'content-read-blob': ['cid'],
  'content-fetch-blob': ['cid'],
  'content-list-blobs': [],
  'content-pin-root': ['cid', 'root'],
  'content-unpin-root': ['cid', 'root'],
  'content-gc-sweep': [],
  // community-affairs §7.2（命令形参 camelCase；Tauri 自动映射到 snake_case）
  'affairs-follow': ['genesis'],
  'affairs-unfollow': ['affairId'],
  'affairs-list-followed': [],
  'affairs-submit-op': ['op'],
  'affairs-read-log': ['affairId'],
  'affairs-read-rules': ['affairId'],
  'affairs-read-resolution': ['affairId'],
  'affairs-ladder-status': ['affairId'],
  'affairs-public-profile': ['identity'],
  // asOf 可选：未传时序列化缺省，壳层 Option<String> 走 None
  'affairs-snapshot-payload': ['affairId', 'asOf'],
  'affairs-read-exec': ['affairId'],
  'affairs-org-effects': ['orgId', 'affairId'],
  'affairs-apply-org-effects': ['orgId', 'affairId'],
  'credentials-list-held': [],
  'credentials-present-holder-proof': ['pluginDomain', 'credId', 'requestId', 'orgId', 'collection'],
  'credentials-query-verifiers': ['orgId'],
  'credentials-verify': ['credential'],
  'credentials-query-revocations': ['issuer'],
  'policy-read': ['orgId'],
  'policy-submit-draft': ['doc'],
  'policy-publish': ['orgId'],
