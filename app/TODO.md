...
（以下已落地，仅保留记录）
- ✅ 已落地：多设备配对 | 设备清单模型已实现（`core/src/device/mod.rs`：`DeviceRecord` 每台设备一条、deviceUid 墓碑替换、`apply_remote`，见协议 §19.4）；`FriendRecord.peers` 列表升级（旧数据自动升级单元素），握手写入首台 + DeviceRecord 聚合，resolve_conv_peer 遍历择优——不再受"每 rootId 至多一台设备"限制
