...
fn derive_release_names(declaration: &SparkPluginDeclaration, id: &RepoId) -> (String, String, String, String) {
    ...
    let manifest = pattern
        .replace("<version>", "manifest")
        .replace(".spkg", ".json");
    let signature = pattern.replace("<version>", "manifest").replace(".spkg", ".sig");
    (
...
impl RepoFetcher for HttpRepoFetcher {
    fn fetch_text(&self, url: &str) -> Result<Option<String>, String> {
        super::sources::fetch_text_http_optional(url)
    }
}

/// 顺序取首个成功（Ok(Some)）；全部 404 → Ok(None)；全部错误 → Err（最后一个错误）。
fn fetch_first(fetcher: &dyn RepoFetcher, sources: &[(SourceFamily, String)]) -> Result<Option<String>, String> {
...
fn fetch_with_cross_check(
    ...
    for (family, url) in sources {
        let Ok(Some(text)) = fetcher.fetch_text(url) else {
            continue;
        };
        match &first {
...
impl PluginMarketService {
    ...
    pub(crate) fn install_from_repo_with(
        ...
        let signature = fetch_first(fetcher, &signature_sources).ok().flatten();

        let (manifest_text, trust_level) = match signature {
            Some(signature_text) => {
                let text = fetch_first(fetcher, &manifest_sources)
                    .ok()
                    .flatten()
        ...
        // 复用现有下载/复制 + sha256/size 校验链路（规格 §5）
...
mod tests {
    ...
    fn cross_check_rules() {
        ...
        assert_eq!(
            fetch_with_cross_check(&fetcher, &sources, true, "MISMATCH", "FETCH"),
            Ok("A".to_string())
        );
    ...
    fn fetch_first_semantics() {
        ...
        assert_eq!(fetch_first(&fetcher, &sources), Ok(Some("hit".to_string())));
        // 全 404 → Ok(None)（资产不存在语义，签名可选层依赖）
        let fetcher = fetcher_of(&[]);
        assert_eq!(fetch_first(&fetcher, &sources), Ok(None));
        // 全错误（无一 404）→ Err
        let fetcher = fetcher_of(&[("u1", "@ERR a"), ("u2", "@ERR b"), ("u3", "@ERR c")]);
        assert_eq!(fetch_first(&fetcher, &sources), Err("@ERR c".to_string()));
    }
}
